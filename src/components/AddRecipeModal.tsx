...
function autoDetectTags(title: string, ingredients: string[], instructions: string[]): string[] {
...
export default function AddRecipeModal({ onClose, onSave, initialRecipe, existingTags }: Props) {
  ...
  const applyScraped = useCallback((data: ScrapedData) => {
    setForm(prev => {
      const detectedTags = autoDetectTags(
        data.title ?? prev.title,
        data.ingredients ?? prev.ingredients.map(i => i.text),
        data.instructions ?? prev.instructions,
      );
      const mergedTags = Array.from(new Set([...prev.tags, ...detectedTags]));
      return {
        ...prev,
  ...
  function applyPaste() {
    ...
    setForm(prev => ({
      ...
      tags: Array.from(new Set([...prev.tags, ...autoDetectTags(
        parsed.title || prev.title,
        parsed.ingredientTexts ?? prev.ingredients.map(i => i.text),
        parsed.instructions ?? prev.instructions,
      )])),
    }));
    setPasteText('');
    setShowPaste(false);
